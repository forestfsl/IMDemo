...
/**
 ...
 @param recent 最近会话
 @return 会话名
 @discussion 默认实现为：点对点会话，显示聊天对象的昵称（没有昵称则显示账号）；群聊会显示群名称
 */
...
/**
 ...
 @param recent 最近会话
 @return 时间戳格式化后的字符串
 @discussion 默认实现为:最后一条消息的时间戳，具体时间戳消息详见NIMKitUtil中, + (NSString*)showTime:(NSTimeInterval) msglastTime showDetail:(BOOL)showDetail 方法的实现。
 */
