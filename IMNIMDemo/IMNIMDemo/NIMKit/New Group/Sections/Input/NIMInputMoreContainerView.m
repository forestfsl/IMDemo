...
- (void)genMediaButtons
{
    ...
    [items enumerateObjectsUsingBlock:^(NIMMediaItem *item, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (UIView *)onLineMediaInPageView:(NIMPageView *)pageView
...
- (UIView *)pageView:(NIMPageView *)pageView viewInPage:(NSInteger)index
{
    if ([_mediaButtons count] == 2 || [_mediaButtons count] == 3) {
        //一行显示两个或者3个
        return [self onLineMediaInPageView:pageView viewInPage:index count:[_mediaButtons count]];
    }
    if (index < 0) {
        assert(0);
        index = 0;
