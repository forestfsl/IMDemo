...
@interface NIMInputEmoticonContainerView()<NIMMEmoticonButtonTouchDelegate,NIMInputEmoticonTabDelegate>
...
#pragma mark NIMPageViewDelegate

- (void)pageViewScrollEnd:(NIMPageView *)pageView currentIndex:(NSInteger)index totalPages:(NSInteger)pages{
    NIMInputEmoticonCatalog *emoticon = [self emoticonWithIndex:index];
    ...
    NSInteger selectTabIndex = [self.totalCatalogData indexOfObject:emoticon];
...
- (NSInteger)sumPages
{
    ...
    [self.totalCatalogData enumerateObjectsUsingBlock:^(NIMInputEmoticonCatalog *data, NSUInteger idx, BOOL * _Nonnull stop) {
