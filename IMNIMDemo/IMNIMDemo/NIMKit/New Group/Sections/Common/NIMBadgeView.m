...
@property (strong) UIColor *badgeBackgroudColor;
...
- (instancetype)initWithFrame:(CGRect)frame{
    if (self = [super initWithFrame:frame]) {
        ...
        _badgeBackgroudColor = [UIColor redColor];
...
- (CGRect)frameWithStr:(NSString *)badgeValue{
    ...
    CGRect badgeFrame = CGRectMake(self.frame.origin.x, self.frame.origin.y, badgeSize.width + self.badgeLeftPadding * 2 + self.whiteCircleWidth * 2, badgeSize.height + self.badgeLeftPadding * 2 + self.whiteCircleWidth * 2 );
    return badgeFrame;
}
...
- (void)drawWithContent:(CGRect)rect context:(CGContextRef)context{
    ...
    if ([self badgeBackgroudColor]) {//外白色描边
        ...
        // badge背景色
        CGContextSetFillColorWithColor(context, [[self badgeBackgroudColor] CGColor]);
        if ([self badgeValue].integerValue > 9) {
