...
+ (NSString *)showTime:(NSTimeInterval)msglastTime showDetail:(BOOL)showDetail
{
    ...
    double OnedayTimeIntervalValue = 24*60*60;  //一天的秒数 result = [NIMKitUtil getPeriodOfTime:hour withMinute:msgDateComponents.minute];
    if (hour > 12)
    {
        hour = hour - 12;
    }
...
+ (NSString *)netcallNotificationFormatedMessage:(NIMMessage *)message{
    ...
    switch (content.eventType) {
        ...
        case NIMNetCallEventTypeReject:{
            text = ([object.message.from isEqualToString:currentAccount]) ?
            @"对方正忙" : @"已拒绝";
            break;
        }
...
+ (NSString *)chatroomNotificationFormatedMessage:(NIMMessage *)message{
    ...
    for (NIMChatroomNotificationMember *member in content.targets) {
        if ([member.userId isEqualToString:[[NIMSDK sharedSDK].loginManager  currentAccount]]) {
            ...
            [targetNicks addObject:member.nick];
        }
    }
...
+ (BOOL)canEditTeamInfo:(NIMTeamMember *)member
{
    ...
    {
        return member.type == NIMTeamMemberTypeOwner || member.type == NIMTeamMemberTypeManager || member.type == NIMTeamTypeNormal;
    }
}
