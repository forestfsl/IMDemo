...
- (void)refreshWithClients:(NSArray *)clients
{
    ...
    if (clients.count) {
     ...
     [self addRow:ListHeaderTypeLoginClients content:text viewClassName:@"IMMutliClientsHeaderView"];
...
- (void)refreshWithCommonText:(NSString *)text{
    [self addRow:ListHeaderTypeCommonText content:text viewClassName:@"IMTextHeaderView"];
}

//by songlin 觉得这里解释不太正确，viewClassName不需要实现IMListHeaderViewDelegate 也可以
//参数viewClassName的Class 必须是UIControl的子类并实现<IMListHeaderViewDelegate>协议
- (void)addRow:(IMListHeaderType)type content:(NSString *)content viewClassName:(NSString *)viewClassName{
    UIControl<IMListHeaderView> *rowView = (UIControl<IMListHeaderView> *)[self viewWithTag:type];
    if ([content length]) {
        if (!rowView) {
...
- (UIColor *)fillBackgroundColor:(IMListHeaderType)type{
    NSDictionary *dict = @{
