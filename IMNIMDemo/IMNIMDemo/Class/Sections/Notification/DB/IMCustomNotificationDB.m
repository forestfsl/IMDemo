...
typedef NS_ENUM(NSUInteger, CustomNotificationStatus) {
...
@interface IMCustomNotificationDB ()
@property (nonatomic, strong) FMDatabase *db;

@end

@implementation IMCustomNotificationDB
...
- (NSArray *)fetchNotifications:(IMCustomNotificationObject *)notification limit:(NSInteger)limit{
    __block NSArray *result = nil;
    NSString *sql = nil;
    if (notification) {
        sql = [NSString stringWithFormat:@"select * from notifications where timetag < %f and status != ? order by timetag desc limit ?",notification.timestamp];
    }else{
    ...
    io_sync_safe(^{
        ...
        while ([rs next]) {
            IMCustomNotificationObject *notification = [[IMCustomNotificationObject alloc]init];
            notification.serial = (NSInteger)[rs intForColumn:@"serial"];
            notification.timestamp = [rs doubleForColumn:@"timetag"];
            notification.sender = [rs stringForColumn:@"receiver"];
            notification.receiver = [rs stringForColumn:@"content"];
...
- (BOOL)saveNotification:(IMCustomNotificationObject *)notification{
    ...
    io_sync_safe(^{
        if (notification) {
            ...
            if (![self.db executeUpdate:sql,@(notification.timestamp),notification.sender,notification.receiver,notification.content,@(status)]) {
...
- (void)markAllNotificationAsRead{
    ...
    io_sync_safe(^{
        if (![self.db executeUpdate:sql,@(CustomNotificationStatusRead),@(CustomNotificationStatusNone)]) {
            DDLogDebug(@"mark notifications read failed");
        }
        [self queryUnreadCount];
    });
}
...
- (void)openDatabase{
    NSString *filepath = [[IMFileLocationHelper userDirectory] stringByAppendingString:@"notification.db"];
...
static const void * const IMDispatchIOSpecifickey = &IMDispatchIOSpecifickey;

dispatch_queue_t IMDispatchIOQueue()
{
    static dispatch_queue_t queue;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        queue = dispatch_queue_create("nim.demo.io.queue", 0);
        dispatch_queue_set_specific(queue, IMDispatchIOSpecifickey, (void *)IMDispatchIOSpecifickey, NULL);
    });
    return queue;
}
...
{
    if (dispatch_get_specific(IMDispatchIOSpecifickey)) {
        block();
    }
    else{
        dispatch_sync(IMDispatchIOQueue(), ^() {
            block();
        });
    }
}
...
void io_async(dispatch_block block){
    dispatch_async(IMDispatchIOQueue(), ^() {
        block();
    });
}
