...
#import "IMSubscribeDefine.h"
#import "IMDevice.h"
#import "NIMExtensionHelper.h"

#define IMSubscribeExpiry 60 * 60 * 24 * 1 //订阅有效期为 1 天

#define IMSubscribeEnable [NIMSDK sharedSDK].isUsingDemoAppKey //仅在使用 Demo App 的时候，将订阅能力开启，开发者可以根据自身应用订阅功能开启状态控制此开关。

NSString *const IMSubscribeNetState = @"net_state";

NSString *const IMSubscribeOnlineState = @"online_state";


@interface IMSubscribeManager()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
@implementation IMSubscribeManager

+ (instancetype)sharedManager
{
    if (IMSubscribeEnable)
    {
        static IMSubscribeManager *instance;
        static dispatch_once_t onceToken;
        dispatch_once(&onceToken, ^{
            instance = [[IMSubscribeManager alloc] init];
        });
        return instance;
    }
    else
...
- (void)publishOnlineState
{
    ...
    event.value = IMCustomStateValueOnlineExt;
    event.sendToOnlineUsersOnly = NO;  //必须要让后登录的用户也能拿到
    NSDictionary *ext = @{
                          IMSubscribeNetState : @([IMDevice currentDevice].currentNetworkType),
                          IMSubscribeOnlineState : @(IMOnlineStateNormal), //移动端永远在线
                          };
...
- (NIMSubscribeRequest *)generateRequest
{
    ...
    request.expiry = IMSubscribeExpiry;
