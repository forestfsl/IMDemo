...
#import "UIActionSheet+IMBlock.h"
...
@interface IMLoginViewController ()<IMRegisterViewControllerDelegate>
...
- (IBAction)onTouchRegister:(id)sender {
    IMRegisterViewController *vc = [IMRegisterViewController new];
    vc.delegate = self;
...
- (void)doLogin
{
    ...
    [[[NIMSDK sharedSDK] loginManager] login:loginAccount
                                  ...
                                  completion:^(NSError *error) {
                                      ...
                                      if (error == nil)
                                      {
                                          IMLoginData *sdkData = [[IMLoginData alloc] init];
                                          sdkData.account   = loginAccount;
                                          sdkData.token     = loginToken;
                                          [[IMLoginManager sharedManager] setCurrentLoginData:sdkData];
...
#pragma mark - Private
- (void)showSDKLog{
    UIViewController *vc = [[IMLogManager sharedManager] sdkLogViewController];
...
- (void)showDemoLog{
    UIViewController *logViewController = [[IMLogManager sharedManager] demoLogViewController];
...
- (void)regisDidComplete:(NSString *)account password:(NSString *)password
{
    if (account.length) {
...
#pragma mark IMRegisterViewControllerDelegate
