...
#import "IMSubscribeManager.h"
#import "NIMExtensionHelper.h"


double OnedayTimeIntervalValue = 24*60*60;  //一天的秒数

static NSString *const IMRecentSessionAtMark  = @"IMRecentSessionAtMark";
static NSString *const IMRecentSessionTopMark = @"IMRecentSessionTopMark";
...
+(NSString*)showTime:(NSTimeInterval) msglastTime showDetail:(BOOL)showDetail
{
    ...
    result = [IMSessionUtil getPeriodOfTime:hour withMinute:msgDateComponents.minute];
    if (hour > 12)
    {
        hour = hour - 12;
    }
    ...
    {
        NSString *weekDay = [IMSessionUtil weekdayStr:msgDateComponents.weekday];
...
+ (NSDictionary *)dictByJsonString:(NSString *)jsonString
{
    ...
    return [IMSessionUtil dictByJsonData:data];
}
...
+ (void)addRecentSessionMark:(NIMSession *)session type:(IMRecentSessionMarkType)type
{
    ...
    if (recent)
    {
        ...
        NSString *key = [IMSessionUtil keyForMarkType:type];
...
+ (NSString *)keyForMarkType:(IMRecentSessionMarkType)type
{
    static NSDictionary *keys;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        keys = @{
                 @(IMRecentSessionMarkTypeAt)  : IMRecentSessionAtMark,
                 @(IMRecentSessionMarkTypeTop) : IMRecentSessionTopMark
                 };
    });
...
{
    NSString *state = @"";
    if (![IMSubscribeManager sharedManager] || [[NIMSDK sharedSDK].loginManager.currentAccount isEqualToString:userId])
    {
    ...
    NSDictionary *dict = [[IMSubscribeManager sharedManager] eventsForType:NIMSubscribeSystemEventTypeOnline];
    ...
    if ([info isKindOfClass:[NIMSubscribeOnlineInfo class]] && info.senderClientTypes.count)
    {
        ...
        switch (event.value) {
            case IMCustomStateValueOnlineExt:
...
+ (NSString *)resolveOnlineState:(NSString *)ext client:(NIMLoginClientType)client detail:(BOOL)detail
{
    ...
    if (dict) {
        ...
        NSString *netState =  [[IMDevice currentDevice] networkStatus:[dict jsonInteger:IMSubscribeNetState]];
        IMOnlineState onlineState =  [dict jsonInteger:IMSubscribeOnlineState];
        switch (onlineState) {
            case IMOnlineStateNormal:
            {
            ...
            case IMOnlineStateBusy:
                return @"忙碌";
            case IMOnlineStateLeave:
                return @"离开";
            default:
                break;
        }
