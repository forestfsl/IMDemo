...
@interface IMBundleSetting : NSObject

+ (instancetype)sharedConfig;

- (BOOL)removeSessionWhenDeleteMessages;    //删除消息时候是否同时删除会话

- (BOOL)dropTableWhenDeleteMessages;    //删除消息的同时是否删除消息表

- (BOOL)localSearchOrderByTimeDesc;     //本地搜索消息书序，YES表示按照时间戳逆序搜索，NO表示按照时间戳顺序搜索
...
- (BOOL)showFps;                    //是否需要显示Fps

- (BOOL)disableProximityMotitor;    //贴耳的时候是否需要自动切换成听筒模式
...
- (BOOL)serverRecordWhioteboardData;        //服务器录制白板数据

- (NIMNetCallVideoCrop)videochatVideoCrop;       //视频画面裁剪比例

- (BOOL)videochatAutoRotateRemoteVideo;     //自动选择视频聊天远端画面
